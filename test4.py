...
departments = {
    ...
    "ร้านค้าสวัสดิการ": ("w1.jpg", "w1.jpg", 210, 3),
...
shortcuts = {
    ...
    "ห้องงานวางแผนและงบประมาณ": ["งานวางแผนและงบประมาณ", "งานวางแผน", "งานงบประมาณ", "วางแผน", "งบประมาณ"],
    "ห้องงานรองผู้อำนวยการแผนและความร่วมมือ": ["งานรองผู้อำนวยการแผนและความร่วมมือ", "งานแผนและความร่วมมือ"],
    "ห้องรองผู้อำนวยการฝ่ายกิจการนักเรียน นักศึกษา": ["รองผู้อำนวยการฝ่ายกิจการนักเรียน", "รองผู้อำนวยการฝ่ายกิจการนักศึกษา"],
    "ห้องรองผู้อำนวยการฝ่ายวิชาการ": ["รองอำนวยการฝ่ายวิชาการ", "รองผู้อำนวยการฝ่ายวิชาการ"],
    "ห้องรองผู้อำนวยการฝ่ายบริหารทรัพยากร": ["รองอำนวยการฝ่ายบริหารทรัพยากร"],
    "ห้องงานอาชีวศึกษาระบบทวิภาคี": ["ทวิภาคี", "ระบบทวิภาคี"],
    "ห้องงานพัฒนาหลักสูตร": ["พัฒนาหลักสูตร"],
    "งานครูที่ปรึกษา": ["งานที่ปรึกษา"],
    "งานปกครอง": ["ปกครอง"],
    "งานแนะแนวและการจัดหางาน": ["แนะแนว", "การจัดหางาน"],
    "งานกิจการนักเรียน นักศึกษา": ["กิจการนักเรียน", "กิจการ"],
    "งานวัดและประเมินผล": ["วัดและประเมินผล"],
...
def show_frame(frame, title=None):
    ...
    if title.startswith("แผนก"):
        ...
        name = title  # name already starts with ห้อง...
        info = departments.get(name)
        if info:
            distance = info[2]
            walk_time = info[3]
...
class AnimatedGIF(ctk.CTkLabel):
    def __init__(self, master, path, width=900, height=600, delay=10, *args, **kwargs):
...
def process_command_text(text):
    txt = text.lower().replace("แผนก", "ห้อง").strip()
    # คำสั่ง 'กลับ' หรือ 'หน้าหลัก'
    if "กลับ" in txt or "หน้าหลัก" in txt:
        # รีเซ็ต last_announced เพื่อให้ประกาศหน้าอื่นได้ใหม่
        with last_announced_lock:
            last_announced["title"] = None
        root.after(0, lambda: show_frame(main_frame, "หน้าหลัก"))
        return

    # หาแผนกตามชื่อหรือ shortcuts (ตรวจแบบ lower-case)
    lowered = txt
    for name in departments.keys():
        if name.lower() in lowered:
            # สร้าง title ให้ถูกต้อง: ถ้า name เริ่มด้วย "ห้อง" ให้ใช้ชื่อเต็ม, มิฉะนั้นใช้ "แผนก"
            title = name if name.startswith("ห้อง") else f"แผนก{name}"
            root.after(0, lambda n=name, t=title: show_frame(image_pages_department[n], t))
            return
...
def create_image_page(root, title, img_path, map_path, distance, walk_time, back_to):
    ...
    header.pack(fill="x")
    try:
        logo_img = Image.open("33.png").resize((90, 90))
        logo_photo = ImageTk.PhotoImage(logo_img)
...
def create_main_menu(root):
    ...
    header.pack(fill="x")
    try:
        logo_img = Image.open("33.png").resize((120, 120))
        logo_photo = ImageTk.PhotoImage(logo_img)
        logo_label = ctk.CTkLabel(header, image=logo_photo, text="")
        logo_label.image = logo_photo
        logo_label.pack(side="left", padx=20, pady=8)
    ...
    float_frame.place(relx=0.02, rely=0.45,)
...
image_pages_department = {
    n: create_image_page(root, (n if n.startswith("ห้อง") else f"แผนก{n}"), v[0], v[1], v[2], v[3], department_frame)
